...
%                                 alternative.
% displayinfo   boolean, true    whether file info will be
...
% -------------------------------------------------------------------------
% determine if we want to display text
global abfload_displayflag % unique name just in case existng global var
vs = varargin;
vs(~cellfun(@ischar,vs)) = deal({''});
dfi = find(~cellfun(@isempty,strfind(vs,'displayinfo')));
if ~isempty(dfi)
    abfload_displayflag = varargin{dfi+1};
else
    abfload_displayflag = 1; % default is true
end
...
machineF='ieee-le';
verbose=1;
% if first input argument is string 'info' the user's
% request is to obtain information on the file (header parameters), so set
% flag accordingly
if strcmp('info',varargin{1})
  doLoadData=false;
else
  doLoadData=true;
...
if eflag
  fclose(fid);
  dispif('**** available channels:');
  dispif(h.recChNames);
  dispif(' ');
  dispif('**** requested channels:');
  dispif(channels);
...
switch h.nOperationMode
  case 1
    dispif('data were acquired in event-driven variable-length mode');
  ...
  case {2,4,5}
    if h.nOperationMode==2
      dispif('data were acquired in event-driven fixed-length mode');
    elseif h.nOperationMode==4
      dispif('data were acquired in high-speed oscilloscope mode');
    else
      dispif('data were acquired in waveform fixed-length mode');
    end
  ...
  case 3
    dispif('data were acquired in gap-free mode');
    ...
    if verbose
      dispif(['total length of recording: ' num2str(tmp,'%5.1f') ' s ~ ' num2str(tmp/60,'%3.0f') ' min']);
      dispif(['sampling interval: ' num2str(h.si,'%5.0f') ' µs']);
      % 8 bytes per data point expressed in Mb
      dispif(['memory requirement for complete upload in matlab: '...
    ...
    if doLoadData
      ...
      elseif length(chInd)/h.nADCNumChannels<1
        ...
        if verbose && nChunk
          dispif(['reading file in ' int2str(nChunk) ' chunks of ~' num2str(chunk) ' Mb']);
        end
  ...
  otherwise
    dispif('unknown recording mode -- returning empty matrix');
    d=[];
    h.si=[];
end
fclose(fid);
...
function dispif(str)
global abfload_displayflag
if abfload_displayflag
    disp(str);
end
